...
class JointRegistrationRegressionTrainer:
    ...
    def train(self, models: dict, datasets: dict, trainer_config=None, full_config=None, device=None, use_tensorboard=False, tensorboard_log_dir=None, early_stop=True, use_wandb=False, enable_wandb_upload=True):
        ...
        # Masking
        masking_pred_disp = used_train_config.get('masking_pred_disp', False)
        ...
        LMA_threshold = trainer_config.get('LMA_threshold', 25)
        ...
        # registration_optimizer = torch.optim.SGD(
        #     registration_model.parameters(), 
        #     lr=used_train_config['optimizers']['registration']['learning_rate'],
        #     # momentum=used_train_config['optimizers']['registration']['momentum'],
        #     weight_decay=used_train_config['optimizers']['registration']['weight_decay'])
        ...
        #     LMA_model.parameters(), 
        #     lr=used_train_config['optimizers']['LMA']['learning_rate'],
        #     # momentum=used_train_config['optimizers']['LMA']['momentum'],
        #     weight_decay=used_train_config['optimizers']['LMA']['weight_decay'])
        ...
        if early_stop:
            early_stop_criteria = used_train_config.get('early_stop_criteria', 'val_loss')
            print(f'Early stop criteria: {early_stop_criteria}')
        ...
        for epoch in progress_bar:
            ...
            for train_batch_idx, train_batch in enumerate(train_loader):
                ...
                if masking_pred_disp:
                    src_mask, tar_mask = train_batch['source_mask'], train_batch['target_mask']
                    src_tar_mask_union = src_mask + tar_mask
                    src_tar_mask_union = src_tar_mask_union.to(used_device)
                    reg_pred_dict['displacement'] = reg_pred_dict['displacement'] * src_tar_mask_union
                ...
                # if LMA_task in ['TOS_regression']:
                #     train_LMA_pred['sector_LMA_labels'] = torch.stack([train_LMA_pred['TOS'] <= LMA_threshold, train_LMA_pred['TOS'] > LMA_threshold], dim=1).to(float)
                    # print(train_LMA_pred['sector_LMA_labels'].shape)
                    # print(train_slice_merged_data['sector_LMA_labels'].shape)
            ...
            # elif LMA_task in ['LMA_sector_classification', 'TOS_regression']:
            elif LMA_task in ['LMA_sector_classification']:
            ...
            with torch.no_grad():
                for batch_idx, val_batch in enumerate(val_loader):
                    ...
                    if masking_pred_disp:
                        src_mask, tar_mask = val_batch['source_mask'], val_batch['target_mask']
                        src_tar_mask_union = src_mask + tar_mask
                        src_tar_mask_union = src_tar_mask_union.to(used_device)
                        pred_dict['displacement'] = pred_dict['displacement'] * src_tar_mask_union
                    ...
                    # if LMA_task in ['TOS_regression']:
                    #     val_LMA_pred['sector_LMA_labels'] = torch.stack([val_LMA_pred['TOS'] <= LMA_threshold, val_LMA_pred['TOS'] > LMA_threshold], dim=1).to(float)
            ...
            if early_stop:
                ...
                if LMA_task == 'LMA_slice_classification':
                    ...
                    raise NotImplementedError(f'LMA task {LMA_task} not implemented')
                curr_epoch_has_improved_loss = epoch_total_val_loss < best_val_loss
                curr_epoch_has_improved_performance = epoch_val_performance > best_val_performance
                if early_stop_criteria == 'val_loss':
                    curr_epoch_has_improved = curr_epoch_has_improved_loss
                elif early_stop_criteria == 'val_performance':
                    curr_epoch_has_improved = curr_epoch_has_improved_performance
                if curr_epoch_has_improved:
    ...
    def test(self, models, datasets, trainer_config=None, full_config=None, device=None, wandb_experiment=None, target_dataset='test'):
        ...
        # build dataloaders
        test_loader = SliceDataLoader(datasets[target_dataset], batch_size=used_train_config['batch_size'], shuffle=True, num_workers=1)
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(test_loader):
                ...
                if masking_pred_disp:
                    src_mask, tar_mask = batch['source_mask'], batch['target_mask']
                    src_tar_mask_union = src_mask + tar_mask
                    src_tar_mask_union = src_tar_mask_union.to(used_device)
                    reg_pred_dict['displacement'] = reg_pred_dict['displacement'] * src_tar_mask_union
                ...
                if LMA_task in ['TOS_regression']:
                    test_LMA_pred['sector_LMA_labels'] = torch.stack([test_LMA_pred['TOS'] <= LMA_threshold, test_LMA_pred['TOS'] > LMA_threshold], dim=1).to(float)
        ...
        print('final_performance_dict: ', test_performance_dict)
