...
class LMATrainer:
    ...
    def train(self, models: dict, datasets: dict, trainer_config=None, full_config=None, device=None, use_tensorboard=False, tensorboard_log_dir=None, early_stop=True, use_wandb=False, wandb_exp=None, exp_save_dir = './test_results', enable_wandb_upload=True, prefix=''):
        ...
        # other parameters
        LMA_modality = used_train_config.get('LMA_modality', 'displacement_field')
        ...
        for epoch in progress_bar:
            ...
            for train_batch_idx, train_batch in enumerate(train_loader):
                # get data
                if LMA_modality == 'displacement_field':
                ...
                elif LMA_modality == 'strain_mat':
                    strain_mat = train_batch['strain_mat'].to(used_device)
                    train_LMA_pred = LMA_model(strain_mat)
                
                # update the prediction dict and the targe dict for the loss calculator
                joint_pred_dict = {
                }
                joint_pred_dict.update(train_LMA_pred)
                # joint_pred_dict['sector_LMA_labels_from_TOS'] = train_LMA_pred['TOS'] > 25
                joint_target_dict = {
                ...
                for loss_name, loss_value in losses_values_dict.items():
                    record_name = f'{prefix}training/{loss_name}'
            ...
            if LMA_task == 'LMA_slice_classification':
                ...
                epoch_loss_dict[f'{prefix}training/LMA_accuracy'] = epoch_training_classification_accuracy
            elif LMA_task == 'LMA_sector_classification':
            ...
            epoch_loss_dict[f'{prefix}validation/total_loss'] = 0
            epoch_val_classification_correct_pred_num = 0
            epoch_val_data_num = 0
            epoch_val_sector_num = 0
            epoch_val_true_positive_num = 0
            epoch_val_pred_positive_num = 0
            epoch_val_actual_positive_num = 0
            epoch_val_raw_preds = []  # for classification only
            epoch_val_all_labels = [] # for classification only
            with torch.no_grad():
                for batch_idx, val_batch in enumerate(val_loader):
                    ...
                    if LMA_modality == 'displacement_field':
                        ...
                        strain_mat = val_batch['strain_mat'].to(used_device)
                        val_LMA_pred = LMA_model(strain_mat)
                    ...
                    if LMA_task == 'LMA_slice_classification':
                        ...
                        # collect raw predictions and labels for computing AUC
                        epoch_val_raw_preds.append(val_LMA_pred['slice_LMA_label'].cpu().numpy())
                        epoch_val_all_labels.append(val_batch['slice_LMA_labels'].cpu().numpy())

                        # data for computing accuracy
                        epoch_val_data_num += val_batch_size
                        epoch_val_classification_correct_pred_num += correct_pred_num
                        
                        # data for computing precision and recall
                        epoch_val_true_positive_num += torch.sum(LMA_pred_class * val_batch['slice_LMA_labels'].to(used_device)).item()
                        epoch_val_pred_positive_num += torch.sum(LMA_pred_class).item()
                        epoch_val_actual_positive_num += torch.sum(val_batch['slice_LMA_labels'].to(used_device)).item()
                    elif LMA_task == 'LMA_sector_classification':
                        # the shape of LMA_pred should be (batch_size, n_classes=2, n_sectors=128)
                        # and the ground truth LMA_labels should be (batch_size, n_sectors=128)
                        ...
                        # collect raw predictions and labels for computing AUC
                        epoch_val_raw_preds.append(val_LMA_pred['sector_LMA_labels'].cpu().numpy())
                        epoch_val_all_labels.append(val_batch['sector_LMA_labels'].cpu().numpy())
                        ...
                        # data for computing precision and recall
                        epoch_val_true_positive_num += torch.sum(LMA_pred_class * val_batch['sector_LMA_labels'].to(used_device)).item()
                        epoch_val_pred_positive_num += torch.sum(LMA_pred_class).item()
                        epoch_val_actual_positive_num += torch.sum(val_batch['sector_LMA_labels'].to(used_device)).item()
                    elif LMA_task == 'TOS_regression':
                        val_batch_size = val_LMA_pred['TOS'].shape[0]
                    else:
                        raise NotImplementedError(f'LMA task {LMA_task} not implemented')
            
            epoch_loss_dict[f'{prefix}validation/total_loss'] = epoch_total_val_loss
            if LMA_task == 'LMA_slice_classification':
                epoch_val_classification_accuracy = epoch_val_classification_correct_pred_num / epoch_val_data_num
                epoch_loss_dict[f'{prefix}validation/LMA_accuracy'] = epoch_val_classification_accuracy
            elif LMA_task == 'LMA_sector_classification':
                epoch_val_classification_accuracy = epoch_val_classification_correct_pred_num / epoch_val_sector_num
                epoch_val_classification_precision = epoch_val_true_positive_num / epoch_val_pred_positive_num
                epoch_val_classification_recall = epoch_val_true_positive_num / epoch_val_actual_positive_num
                # epoch_val_raw_preds_mean_tensor = torch.Tensor(np.mean(epoch_val_raw_preds, axis=0))
                # epoch_val_all_labels_tensor = torch.Tensor(np.concatenate(epoch_val_all_labels, axis=0))
                # epoch_val_classification_AUC = roc_auc_score(epoch_val_all_labels_tensor, epoch_val_raw_preds_tensor)
                epoch_loss_dict[f'{prefix}validation/LMA_accuracy'] = epoch_val_classification_accuracy
                epoch_loss_dict[f'{prefix}validation/LMA_precision'] = epoch_val_classification_precision
                epoch_loss_dict[f'{prefix}validation/LMA_recall'] = epoch_val_classification_recall
                # epoch_loss_dict['validation/LMA_AUC'] = epoch_val_classification_AUC
            elif LMA_task == 'TOS_regression':
                pass
            ...
            if use_wandb and enable_wandb_upload:
                ...
                if wandb_visualize_interval > 0 and epoch % wandb_visualize_interval == 0:
                    ...
                    # }, step=epoch)
                    wandb_experiment.log({
                        f'{prefix}training/LMA_visualization': wandb.Image(train_fig),
                        f'{prefix}validation/LMA_visualization': wandb.Image(val_fig),
                    }, step=epoch)
        ...
        exp_dict = {
            ...
            f'{prefix}epoch_loss_dict': self.epoch_loss_dict,
            f'{prefix}LMA': LMA_model,
        }
    ...
    def test(self, models, datasets, trainer_config=None, full_config=None, device=None, wandb_experiment=None, target_dataset='test', prefix=''):
        ...
        print(f'len(datasets[{target_dataset}]): ', len(datasets[target_dataset]))
        test_loader = DataLoader(datasets[target_dataset], batch_size=used_train_config['batch_size'], shuffle=False, num_workers=1)
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(test_loader):
                ...
                if LMA_modality == 'displacement_field':
                    ...
                    strain_mat = batch['strain_mat'].to(used_device)
                    test_LMA_pred = LMA_model(strain_mat)
                    batch_size = strain_mat.shape[0]
        ...
        if LMA_task == 'LMA_slice_classification':
            ...
            test_performance_dict[f'{prefix}final-{target_dataset}/LMA_accuracy'] = test_classification_accuracy
        elif LMA_task == 'LMA_sector_classification':
