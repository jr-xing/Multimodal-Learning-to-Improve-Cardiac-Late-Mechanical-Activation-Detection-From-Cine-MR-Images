...
class LMADataset(TorchDataset):
    def __init__(self, data, augmentation=None, dataset_config = {}, full_config={}, dataset_name=None):
        ...
        self.n_frames_for_strain_matrix = self.dataset_config.get('n_frames_for_strain_matrix', self.n_frames_to_use_for_regression)
    ...
    def align_displacement_field_frames(self):
        ...
        print(f'# of frames before alignment: {list(set([d[self.displacement_field_key + "_X"].shape[-1] for d in self.data]))}')
        ...
        print(f'# of frames after alignment: {list(set([d[self.displacement_field_key + "_X"].shape[-1] for d in self.data]))}')
    ...
    def align_strain_mat_frames(self):
        ...
        for datum_idx, datum in enumerate(self.data):
            self.data[datum_idx]['strain_matrix'] = align_n_frames_to(datum['strain_matrix'], self.n_frames_for_strain_matrix, frame_idx, padding_method)
    ...
    def __getitem__(self, index):
        ...
        datum['displacement_field_X'] = torch.from_numpy(raw_datum[self.displacement_field_key + '_X'][None, ...]).to(torch.float32)
        datum['displacement_field_Y'] = torch.from_numpy(raw_datum[self.displacement_field_key + '_Y'][None, ...]).to(torch.float32)
